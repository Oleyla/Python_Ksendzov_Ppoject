...
#                     значении переменной currency_result высчитать сколько долларов получится при target_currency_amount и usd_eur_rate. Результат вывести в консоль
#                     (target_currency_amount, eur_item, “=”, currency_result, usd_item)
#                     31.5 Сделать elif в котором будет условие: если target_currency равен ‘uah’, то в теле этого if в значении переменной currency_result высчитать сколько долларов
#                     получится при target_currency_amount и usd_uah_rate. Результат вывести в консоль (target_currency_amount, uah_item, “=”, currency_result, uah_item)
...
target_currency = input('Введите название валюты для конвертации: ')
if currency_convertor:
    if target_currency == 'eur':
        target_currency = eur_item
        target_currency_amount = input('Введите сумму для обмена: ')
        currency_result = float(target_currency_amount) / usd_eur_rate
        print ('Итоговый результат: '+str(target_currency_amount )+' '+eur_item+' = '+str(currency_result)[:8]+' '+usd_item,)
    elif target_currency == 'uah':
        target_currency = uah_item
        target_currency_amount = input('Введите сумму для обмена: ')
        currency_result = float(target_currency_amount) / usd_uah_rate
        print('Итоговый результат: '+str(target_currency_amount) + ' ' + uah_item + ' = ' + str(currency_result)[:8] + ' ' + usd_item, )
    elif target_currency == 'chf':
        target_currency = chf_item
        target_currency_amount = input('Введите сумму для обмена: ')
        currency_result = float(target_currency_amount) / usd_chf_rate
        print('Итоговый результат: ' + str(target_currency_amount) + ' ' + chf_item + ' = ' + str(currency_result)[:8] + ' ' + usd_item, )
    elif target_currency == 'rub':
        target_currency = rub_item
        target_currency_amount = input('Введите сумму для обмена: ')
        currency_result = float(target_currency_amount) / usd_rub_rate
        print('Итоговый результат: ' + str(target_currency_amount) + ' ' + rub_item + ' = ' + str(currency_result)[:8] + ' ' + usd_item, )
    elif target_currency == 'byn':
        target_currency = byn_item
        target_currency_amount = input('Введите сумму для обмена: ')
        currency_result = float(target_currency_amount) / usd_byn_rate
        print('Итоговый результат: ' + str(target_currency_amount) + ' ' + byn_item + ' = ' + str(currency_result)[:8] + ' ' + usd_item, )
    else:
        print('Неизвестная валюта. К сожалению, обмен данной валюты невозможен!')
else:
